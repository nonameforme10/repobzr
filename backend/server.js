...
// Database Clear / Reset Endpoint (Clears transactions & history, preserves products & categories)
apiRouter.post('/reset-database', async (req, res, next) => {
  const client = await db.getClient();
  try {
    await client.query('BEGIN');

    // Truncate activities and sync operations (products & categories are strictly preserved)
    await client.query(`
      TRUNCATE TABLE activities, sync_operations RESTART IDENTITY CASCADE
    `);

    // Truncate sales and sale_items if tables exist
    await client.query(`
      DO $$ 
      BEGIN 
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'sales') THEN
          TRUNCATE TABLE sales, sale_items RESTART IDENTITY CASCADE;
        END IF;
      END $$;
    `);

    // Restart changes sequence and re-populate with existing categories & products
    await client.query(`
    ...
    const catRes = await client.query(
      'SELECT id, name, version, created_at AS "createdAt", updated_at AS "updatedAt" FROM categories WHERE is_deleted = FALSE ORDER BY created_at ASC'
    );
    for (const cat of catRes.rows) {
      await client.query(
        'INSERT INTO changes (entity_type, entity_id, action, data, created_at) VALUES ($1, $2, $3, $4, $5)',
        ['category', cat.id, 'CREATE', cat, now]
      );
    }

    const prodRes = await client.query(
      'SELECT id, category_id AS "categoryId", name, quantity::float, sold::float, price::float, notes, image, translations, version, created_at AS "createdAt", updated_at AS "updatedAt" FROM products WHERE is_deleted = FALSE ORDER BY created_at ASC'
    );
    for (const prod of prodRes.rows) {
      await client.query(
        'INSERT INTO changes (entity_type, entity_id, action, data, created_at) VALUES ($1, $2, $3, $4, $5)',
        ['product', prod.id, 'CREATE', prod, now]
      );
    }

    const seqRes = await client.query('SELECT COALESCE(MAX(seq), 0)::bigint AS max_seq FROM changes');
    const currentSeq = Number(seqRes.rows[0]?.max_seq || 0);

    // Update settings (preserve existing currency)
    const generalRes = await client.query("SELECT value FROM settings WHERE key = 'general'");
    let generalVal = { currency: 'UZS' };
    if (generalRes.rows.length > 0 && generalRes.rows[0].value) {
      generalVal = typeof generalRes.rows[0].value === 'string' ? JSON.parse(generalRes.rows[0].value) : generalRes.rows[0].value;
    }
    generalVal.lastResetDate = now;
    await client.query(
      "INSERT INTO settings (key, value, updated_at) VALUES ('general', $1, $2) ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = $2",
      [JSON.stringify(generalVal), now]
    );

    await client.query('COMMIT');
    console.log(`[db] Database reset executed successfully (products preserved). currentSeq: ${currentSeq}`);
    res.status(200).json({ ok: true, message: 'Database reset successfully (products preserved)', currentSeq });
  } catch (err) {
